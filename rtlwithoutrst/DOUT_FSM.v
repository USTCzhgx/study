...
module DOUT_FSM (
    ...
    // active_id is consumed only in DOUT_SEND.  The transition into that
    // state captures a valid non-EOP grant first, so no reset is required.
    always @(posedge i_clk) begin
        if ((dout_state == DOUT_IDLE) &&
            i_grant_vld &&
            selected_vld &&
            !selected_eop) begin
            active_id <= grant_id;
